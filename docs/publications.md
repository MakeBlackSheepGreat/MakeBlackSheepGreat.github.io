...
为提升文献调研效率构建本地知识库：以内容哈希保证幂等、任务表驱动断点续跑、本机 GPU 与云端推理混合调度。共处理论文 30,031 篇，完成 30,018 篇（99.96%），用时 11.2 小时（按完成量折算约 1.3 秒/篇）；每条知识条目均建立指向原文的证据链接，配合待审队列由人工复核后转正，保证条目可溯源。
